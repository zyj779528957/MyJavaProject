...
String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+"/";
	...
	try {
		...
		/* SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		   ...
		   tNews.setNmodifydate(new Date()); //修改时间 */
		...
		for (FileItem item : list) {
			...
			if (item.isFormField()) {
				...
				if(name.equals("nid")){
					tNews.setNid(Integer.parseInt(item.getString("UTF-8"))); //新闻ID
					System.out.println("str-nid:"+Integer.parseInt(item.getString("UTF-8")));
		...
		if(flag){
			...
			Integer count = tns.update(tNews); //调用业务层新闻实现类中的update有参方法（实质后台业务层中的此方法紧接着再进一步调用数据层中的修改方法）
